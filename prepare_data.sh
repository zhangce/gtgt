...
psql -U $DB_USER -c "CREATE TABLE top_features (id bigserial primary key, feature text);" $DB_NAME
...
psql -U $DB_USER -c "CREATE TABLE relations_partial (id bigserial primary key, \
											 feature1  text,               \
											 feature2  text,               \
											 _per_children  boolean,               \
											 _per_parents  boolean,               \
